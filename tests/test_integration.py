...
def test_basic_workflow(temp_repo: Path, capsys: CaptureFixture[str]) -> None:
    ...
    commit_text = obj["content"].decode()
    assert "Initial commit" in commit_text
    assert "tree " in commit_text


def test_multiple_commits_workflow(
...
def test_empty_commit_prevention(temp_repo: Path, capsys: CaptureFixture[str]) -> None:
...
def test_file_modification_workflow(temp_repo: Path) -> None:
    ...
    assert second_commit is not None  # Ensure it's not None before reading
    obj = read_object(second_commit)
    commit_text = obj["content"].decode()
    assert f"parent {first_commit}" in commit_text
